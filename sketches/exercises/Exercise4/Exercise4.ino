/*
  Exercise 4: Read LDR light intensity from A0 and show it on an LCD
  ...
  The percentage is a relative light level, not a calibrated lux value.
  If your module's percentage falls as light increases, change
  REVERSE_LIGHT_SCALE to true.
...
void loop() {
  ...
  if (REVERSE_LIGHT_SCALE) {
    lightPercent = map(raw, 4095, 0, 0, 100);
  } else {
